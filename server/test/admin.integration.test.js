...
describe("administração", skipWithoutDatabase, () => {
  ...
  test("promover e despromover, com registo de quem o fez", async () => {
    ...
    assert.equal(historial.body.changes[0].by, (await admin.get("/api/users/me")).body.user.username);
  });
  ...
  test("promover quem já é moderador não escreve uma segunda linha no historial", async () => {
    ...
    const { rows } = await query(`SELECT count(*)::int AS n FROM role_changes WHERE target_id = $1`, [
      pessoaId,
    ]);
  ...
  test("a lista filtra por papel e procura por nome ou email", async () => {
    ...
    assert.deepEqual(
      comPapel.body.users.map((u) => u.role).sort(),
      ["admin", "moderator"],
    );
  ...
  test("apagar a conta de quem promoveu não apaga o historial da promoção", async () => {
    ...
    assert.equal(rows[0].actor_id, null, "o registo fica, sem apontar para uma conta que já não há");
  });
